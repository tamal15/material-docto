...
const AvailableAppointment = ({date}) => {
    ...
    const fetchData = () => {
        fetch(`http://localhost:5000/getDoctors?page=${page}&&sizing=${sizing}&&size=${size}&&specialist=${specialist}`)
        .then(res => res.json())
        .then(data => {
...
const  handleOnChange=(e)=>{
    e.preventDefault()
    const values = e.target.value;
    const newValue = questions?.filter(ques => ques?.division?.toLowerCase()?.includes(values.toLowerCase()))
    // console.log(values)
    newValue.length === 0 && alert("warning", "Warning...", "Not Found Your Result")
    setModel(newValue)
}

const placeholder = 'Search by  Division Example:Dhaka';
    return (
        <div>
                         ...
                         {/* {
                          model.map((models)=>( */}
                           ...
                           <div className='brands mt-3'>
                            <h5 style={{fontWeight:"700",fontSize:"25px"}} className='text-black texts-design'>Treatment</h5>
                           <div className="form-check align-items-center me-3">
                                <input className="form-check-input" type="checkbox" value="TeethOrthodonics" id="flexCheckDefault" />
                                <label style={{fontSize:"18px"}} className="form-check-label fw-bold all text-black me-4" for="flexCheckDefault">
                                TeethOrthodonics
                                </label>
                            </div>
                            <div className="form-check align-items-center me-3">
                                <input className="form-check-input" type="checkbox" value="CosmeticDentistry" id="flexCheckDefault" />
                                <label style={{fontSize:"18px"}} className="form-check-label fw-bold all me-4 text-black" for="flexCheckDefault">
                                CosmeticDentistry
                                </label>
                            </div>
                            <div className="form-check align-items-center me-3">
                                <input className="form-check-input" type="checkbox" value="TeethCleaning" id="flexCheckDefault" />
                                <label style={{fontSize:"18px"}} className="form-check-label fw-bold all me-4 text-black" for="flexCheckDefault">
                                TeethCleaning
                                </label>
                            </div>
                            <div className="form-check align-items-center me-3">
                                <input className="form-check-input" type="checkbox" value="CavityProtection" id="flexCheckDefault" />
                                <label style={{fontSize:"18px"}} className="form-check-label fw-bold all me-4 text-black" for="flexCheckDefault">
                                CavityProtection
                                </label>
                            </div>
                            <div className="form-check align-items-center me-3">
                                <input className="form-check-input" type="checkbox" value="PediatricDental" id="flexCheckDefault" />
                                <label style={{fontSize:"18px"}} className="form-check-label fw-bold all me-4 text-black" for="flexCheckDefault">
                                PediatricDental
                                </label>
                            </div>
                            <div className="form-check align-items-center me-3">
                                <input className="form-check-input" type="checkbox" value="OralSurgery" id="flexCheckDefault" />
                                <label style={{fontSize:"18px"}} className="form-check-label fw-bold all me-4 text-black" for="flexCheckDefault">
                                OralSurgery
                                </label>
                            </div>
