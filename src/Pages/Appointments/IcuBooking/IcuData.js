...
import IcuBookData from './IcuBookData';
...
const IcuData = ({date}) => {
    ...
    const fetchData = () => {
        fetch(`http://localhost:5000/getIcu?page=${page}&&sizing=${sizing}&&size=${size}&&division=${division}`)
        .then(res => res.json())
        .then(data => {
      ...
      useEffect(()=>{
        fetch('http://localhost:5000/getIcu')
        .then(res=>res.json())
...
const  handleOnChange=(e)=>{
    e.preventDefault()
    const values = e.target.value;
    const newValue = questions?.filter(ques => ques?.HospitalName?.toLowerCase()?.includes(values.toLowerCase()))
    // console.log(values)
    newValue.length === 0 && alert("warning", "Warning...", "Not Found Your Result")
    setModel(newValue)
}

const placeholder = 'Search by  HospitalName Example:Dhaka';
    return (
        <div>
                         ...
                         {/* {
                          model.map((models)=>( */}
                           ...
                           <div className='brands mt-3'>
                            <h5 style={{fontWeight:"700",fontSize:"28px"}} className='text-black texts-design'>Division</h5>
                           <div className="form-check align-items-center me-3 mt-3">
                                <input className="form-check-input" type="checkbox" value="Dhaka" id="flexCheckDefault" />
                                <label style={{fontSize:"26px"}} className="form-check-label fw-bold all text-black me-4" for="flexCheckDefault">
                                Dhaka
                                </label>
                            </div>
                            <div className="form-check align-items-center me-3 mt-3">
                                <input className="form-check-input" type="checkbox" value="Rangpur" id="flexCheckDefault" />
                                <label style={{fontSize:"26px"}} className="form-check-label fw-bold all me-4 text-black" for="flexCheckDefault">
                                Rangpur
                                </label>
                            </div>
                            <div className="form-check align-items-center me-3 mt-3">
                            ...
                            <div className="form-check align-items-center me-3 mt-3">
                                <input className="form-check-input" type="checkbox" value="Khulna" id="flexCheckDefault" />
                                <label style={{fontSize:"26px"}} className="form-check-label fw-bold all me-4 text-black" for="flexCheckDefault">
                                Khulna
                                </label>
                            </div>
                            <div className="form-check align-items-center me-3 mt-3">
                                <input className="form-check-input" type="checkbox" value="sylhet" id="flexCheckDefault" />
                                <label style={{fontSize:"26px"}} className="form-check-label fw-bold all me-4 text-black" for="flexCheckDefault">
                                sylhet
                                </label>
                            </div>
