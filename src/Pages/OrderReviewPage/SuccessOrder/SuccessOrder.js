...
const SuccessOrder = () => {
    ...
    useEffect(() => {
        fetch(`http://localhost:5000/payorders/${id}`)
            .then(res => res.json())
            .then(data => {
                setBook(data);
                console.log(data)
    ...
    const handlerGotoHome = () => history.replace('/');
    const handlerGoToMyOrder = () => history.replace('/dashboard/myOrder');

    let width = window.innerWidth;
    let height = window.innerHeight;
    return (
        <Box sx={{ overflow: 'hidden' }} className="data-background">
            {/* <NavigationBar /> */}
